...
// #2637: this file used to be a classic `<script>` in admin.html publishing
// `window._ttsConfig`, which admin.js and admin/sections/game-settings.js then
// read back — a page-global handshake that only worked because of the order of
// the two <script> tags. It is now an ES module imported by admin.js, so it is
// part of the admin bundle and `npm run build:check` sees it (the #1263 class of
// drift). `window.BeatifyTtsPresets` stays a global on purpose: its consumer is
// wizard.js, which the page loads as its own module entry point and which
// therefore cannot import from the admin bundle.
...
// #2637: same DOM-less-import guard as the auto-init at the bottom.
...
/**
 * The TTS block of the start-game / update-lobby payload.
 *
 ...
 * carries the stale page-load defaults (enabled:false) and the backend skips
 * configure_tts, leaving the game silent.
 *
 * #2637: was `window._ttsConfig`; admin.js and game-settings.js import it now.
 *
 * @returns {Object} enabled/entity_id/tts_pre_round_delay plus the 23 booleans
 */
